...
### Changed
- Workspace package names renamed from `@yields/*` to `@risu/*` (root package `yields` → `risu`), matching the product's actual name. No behaviour change — `pnpm --filter @risu/core build` replaces `@yields/core`, etc. `YIELDS_DB_PATH` and `data/yields.db` are unchanged (a separate, runtime-facing rename, not done here).
- Also removed `docs/` from the repo (and its git history) — it had accumulated real local filesystem paths from internal planning notes; the supported-import-formats reference it held now lives inline in README.md.
- **Tax tab minimize-CGT leaves Betashares Direct statement sells on FIFO.** That platform issues its own tax report using FIFO (including auto-rebalance sells), so Risu no longer re-identifies those imported sells when Auto (minimize CGT) is on. A sale you confirm on the ticker page — including a manual Betashares Direct sell — still uses the parcel mix you chose. Expandable FY rows show a **Parcels** column (FIFO / Min CGT / Recorded).
- **Configurable FIFO-lock brokers:** Settings → Tax lists which brokers’ imported statement sells stay FIFO (default: Betashares Direct). Untick to let Auto (minimize CGT) apply; Add trade and ticker Confirm sale are never locked. Stored as `platform_fifo_brokers`.
- **Stake DRP page:** clearer explanation of why Stake Activity hides reinvestment, which files to drop, Analyze vs Confirm, and that non-DRP gaps are warnings only. Files you run through the check are stored in import history (source `stake-drp-detect`) and listed on that page like Import file — they stay off the regular Import-file history.
- **Stake DRP Analyze lists duplicates:** suggested DRP rows already in the ledger (same Stake DRP id, or same date/ticker/units as an existing DRP) show under **Already in the ledger — will skip**, so a re-run is visibly a no-op for those lots. Confirm only inserts the new ones.

### Added
- **Valuations tab — stamp what everything was worth on a date.** Pick any date up to today (defaults to the last 30 June) and a portfolio; Risu values every holding at the last cached close on or before it, **per portfolio → broker → stock**, down to each parcel's cost base and value. Parcels are worked out per broker, so a sell at one broker never consumes another broker's parcels; a split row with no broker is shared across the brokers holding the stock then, pro rata. Each line shows the close date used (and the FX rate and its date for US holdings), and flags anything a tax record shouldn't silently rely on: no price, a price more than 5 days old, today's FX used for a past date, no FX, or units with no parcel cost. **Save** stores the report whole in the new `valuation_snapshots` table — recomputed on save, then never changed by later price refreshes or ledger edits — with a label and notes; saved records can be reopened, downloaded as CSV (one row per parcel) or deleted. Built for 30 June 2027, when the CGT rules split every gain at each parcel's market value just before 1 July 2027. Totals match the performance chart for the same date. `GET /api/valuations/preview`, `POST/GET/DELETE /api/valuations`, `GET /api/valuations/:id/csv`; `valuationAsOf` in `@risu/core`.
- **Parcels as of a date (`GET /api/lots?portfolioId=&asOf=&since=`):** read-only. Returns every open parcel on `asOf` with its AUD cost base, market value, and value at the 1 Jul 2027 cutover, plus the buys and matched sells between `since` and `asOf`. Sells match parcels the same way the Tax tab does. Built so Tanuki's plan tracker can use real parcels instead of typed share balances; `lotsAsOf` in `@risu/core`.
- **Transactions table opens the ticker page:** click a row (ticker styled like Holdings) to go to that stock's page, same as clicking a holding. Checkboxes still select for delete.
- **Ticker page — “If you sell” CGT estimate:** enter how many units to sell and pick **FIFO** or **Minimize CGT**. Minimize CGT chooses which remaining parcels to dispose of to reduce estimated tax (losses and smallest post-discount / post-indexation gains first). Sale date drives the 1 Jul 2027 cutoff: before it, parcels held ≥ 12 months get the 50% discount; from that date, cost is CPI-indexed and there is no discount (rate floored at 30%). Shows estimated tax, gain, the parcels that would be sold, and a comparison vs the other matching method. `GET /api/tax/sell-estimate`.
- **Tax tab parcel matching:** primary choice is now **Auto (FIFO)** vs **Auto (minimize CGT)** — the latter re-identifies which parcels each historical sell consumes to reduce estimated CGT (specific identification), instead of always oldest-first. The existing 50% discount and indexed regime overrides stay as reference. `lotMatching=fifo|min_cgt` on `GET /api/tax/fy-estimate`.
- **Confirm a parcel-identified sale:** the ticker “If you sell” estimate can be written to the ledger (`POST /api/tax/confirm-sale`). That stores which buy/DRP parcels were fully or partially consumed (and when) in `parcel_disposals`. Later CGT estimates use those recorded parcels instead of guessing FIFO again. Transactions lists show **Sold** / **Partial · N left** on the original buy/DRP, and **FIFO/Min CGT · N parcels** on the sell (hover for acquired dates). Delete the sell first if you need to undo; deleting a recorded buy while its sell still exists is blocked.

### Fixed
- Parcel CGT tables no longer label short-term lots as “50% disc.” The pre–Jul 2027 ruleset still applies, but the 50% discount is only used when held ≥ 365 days — short-term lines now read **No disc. (short)**.

### Added
- Daily WAL-safe sqlite snapshots from hana-server to `~/Documents/Finances/risu-backups` (`scripts/install-hana-backup.sh`). Live file on the server is `~/risu-data/risu.db`.
- `scripts/deploy-hana.sh` builds and runs Risu on hana-server via Podman Quadlet (host network, port **8788** so it does not collide with crypto-tax on 8787). Caddy serves **http://risu.hana-server/**; the app binds `127.0.0.1`. Runbook: `docs/hana-server.md`.
- **FY tax estimate, its own top-level "Tax" tab:** `GET /api/tax/fy-estimate` and a new panel showing roughly how much tax is owed for a financial year — dividend income tax plus realised CGT on actual sells, combined. Capital gains use real **FIFO per-parcel cost base** (`computeLots` in `@yields/core`) instead of average cost: each sold parcel keeps its own acquisition date, so the 1 Jul 2027 CGT rule change (50% discount pre-cutover vs CPI-indexed/no-discount/30%-floor post-cutover, `auto_by_date` regime) applies per-disposal rather than to the whole holding at once. **Gains and losses net within each FY before tax is computed** (`tax/realisedCgt.ts`): a loss on one parcel offsets a gain on another sold the same year — under the 50%-discount regime, losses are applied to non-discount-eligible (short-term) gains first, the order most favourable to the taxpayer, so a discount-eligible gain is only reduced by whatever loss remains. A net loss for the FY is $0 tax and is surfaced explicitly as `netCapitalLoss` rather than silently discarded (see below — it now carries forward to a later FY). Expandable per-FY rows show each disposal's ticker, acquired/disposed dates, quantity, proceeds, cost base, and raw gain/loss (a per-line "tax" figure was removed — it isn't meaningful once gains/losses net at the FY level). This is separate from the planner's `estimateLiquidationCgt` (a hypothetical single-lot exit) — it reads the real ledger. Started under Settings, moved to its own nav tab, and gained an explicit **Refresh** button (it doesn't auto-poll for ledger changes — e.g. a manually-added split/correction — so a manual refresh re-fetches on demand).
- **Three new Holdings-tab charts:** **Allocation** (horizontal bar, market value by ticker, top 10 + "Other" — a ranked bar rather than a pie, which stops being legible past a handful of similar-sized slices); **Realised vs unrealised gain** by FY (grouped bar; realised = actual sells via FIFO cost base, tax-agnostic — see new `summarizeRealisedGains`/`summarizeFyEndSnapshots`/`combineGainsByFy` in `@yields/core`, and `GET /api/gains/by-fy`); **Dividend income** by FY (single-series bar, total assessable income — reusing the existing `GET /api/income`, no UI tab revived). Colors use a validated CVD-safe categorical order (dataviz skill) rather than ad hoc hex picks.
- `income.ts`'s FY totals additionally expose `amountCashAud`/`amountDrpAud` (cash-vs-DRP dollar split) for future use — not surfaced in the chart above: most real DRP rows (SelfWealth/Stake-style imports) carry no disclosed reinvestment amount (`price`/`amount` null by design), so a $ split on this ledger's data reads as "always ~$0 DRP" regardless of the ledger; a unit-count-based DRP view would be the next step if this is wanted later.
- **Tax tab now has its own Portfolio selector:** previously the Tax tab silently reused whatever portfolio was last selected on Holdings, with no visible control on the tab itself — switching "Tax profile" (Me/Partner, a marginal-rate setting) looked like it should also switch whose stocks were being analysed, but the two are unrelated concepts (a tax profile has no link to a portfolio in the data model). The Tax tab now shows an explicit **Portfolio** dropdown (syncing with the same app-wide filter used on Holdings/Transactions) alongside Tax profile, with a one-line explanation that Portfolio picks whose stocks to analyse and Tax profile picks whose marginal rate to apply.
- **Capital loss carry-forward across FYs:** a net capital loss no longer just sits at $0 tax for its own FY and disappears — `estimateRealisedCgtForLedger` now processes FYs oldest-first, threading an unused-loss balance forward and applying it against a later FY's net gain (short-term/non-discount-eligible gains first, same taxpayer-favourable order as within-FY netting; regime-agnostic, so a `discount_50` loss carries into an `indexation_min30` FY the same way). New per-FY fields `lossCarriedIn`/`priorLossApplied`/`lossCarriedOut` (core) and `priorLossApplied`/`lossCarriedForward` (the combined FY report) make this visible rather than implicit — the Tax tab's expanded FY view now shows "$X applied from an earlier year's loss" and "$Y carried forward to a future FY" alongside the existing net-loss note. 4 new tests cover full/partial absorption, short-term-first application, and carrying across the 2027 regime cutover. The main FY table also gained a **"Loss carried fwd"** column showing the rolling accumulated balance per FY (e.g. FY2026 reads `$2,617.89` with a `$2,015.65 + $602.24 prior` breakdown beneath it), so the running total is visible without expanding every row.
- **Tax estimate panel clarity pass:** the "Total tax" column is renamed **"Est. tax payable"** and no longer styled green (green read as "good news" for what is, in fact, a bill); it's now a neutral bold chip so it doesn't compete visually with the red-highlighted net-loss figures next to it. The current FY gets a **"Current"** badge and a subtle row highlight. Expanding a FY row now also shows the underlying **dividend income lines** (`income.ts`'s new `events`/`AssessableDividendEvent[]`, threaded through `estimateFyTax` as `dividendEvents`) alongside the CGT parcels — previously only the CGT side was shown, so a FY with tax but no capital gains (e.g. dividend-only, or a net capital loss) looked unexplained.
